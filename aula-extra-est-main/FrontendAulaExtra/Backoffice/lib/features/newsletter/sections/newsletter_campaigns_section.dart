...
class NewsletterCampaignsSection extends StatelessWidget {
  const NewsletterCampaignsSection({required this.items, super.key});
            ...
            builder: (context, constraints) {
                ...
                return Column(
                  children: [
                    for (var index = 0; index < items.length; index++) ...[
                      _CampaignMobileCard(item: items[index]),
              ...
              return Column(
                children: [
                  ...
                  for (var index = 0; index < items.length; index++) ...[
                    _CampaignTableRow(item: items[index]),
...
class _CampaignTableRow extends StatelessWidget {
  const _CampaignTableRow({required this.item});
      ...
      child: Row(
        children: [
          ...
          Expanded(
            flex: 2,
            child: Text(
              item.status == NewsletterCampaignStatus.sent
                  ? '${item.openRate.toStringAsFixed(1)}% open · ${item.clickRate.toStringAsFixed(1)}% CTR'
                  : '${item.sentCount} contactos',
              style: const TextStyle(
                color: AppColors.textSecondary,
                fontSize: 14,
                fontWeight: FontWeight.w600,
...
class _CampaignMobileCard extends StatelessWidget {
  const _CampaignMobileCard({required this.item});
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            item.status == NewsletterCampaignStatus.sent
                ? 'Abertura ${item.openRate.toStringAsFixed(1)}% · CTR ${item.clickRate.toStringAsFixed(1)}%'
                : '${item.sentCount} contactos previstos',
            style: const TextStyle(
              color: AppColors.textSecondary,
              fontSize: 14,
              fontWeight: FontWeight.w600,
