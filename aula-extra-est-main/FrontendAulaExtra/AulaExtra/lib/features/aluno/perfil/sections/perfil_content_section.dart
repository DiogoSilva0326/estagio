...
class _PerfilContentSectionState extends State<PerfilContentSection> {
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      final parts = displayName.split(RegExp(r'\s+')).where((p) => p.trim().isNotEmpty).toList(growable: false);
      ...
      setState(() {
        ...
        _telefoneController.text = (me.mobileNumber ?? me.phoneNumber ?? '').trim();
  ...
  Future<void> _removeDisciplina(DisciplinaDto disciplina) async {
    ...
    try {
      final updated = await _education.removeMyDisciplina(idDisciplina: disciplina.idDisciplina);
      if (!mounted) return;
      setState(() {
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const Text('Gerencie suas informações pessoais', style: PerfilConstants.subtitleStyle),
                ...
                if (_loadingProfile)
                  ...
                  Row(
                    children: [
                      Expanded(child: Text(_profileError!, style: const TextStyle(color: Colors.red))),
                      const SizedBox(width: 12),
                      OutlinedButton(onPressed: _loadProfile, child: const Text('Tentar novamente')),
                    ],
                  )
                else
                _PerfilFormCard(
                  ...
                  onNivelEnsinoChanged: (value) => setState(() => _nivelEnsino = value),
                  ...
                  onInteressesVerMais: () => setState(() => _interessesPagesShown += 1),
                  onReloadDisciplinas: _loadMyDisciplinas,
                  onRemoveDisciplina: _removeDisciplina,
                  onSalvar: () async {
                    try {
                      final computedDisplayName =
                          '${_primeiroNomeController.text.trim()} ${_ultimoNomeController.text.trim()}'.trim();
...
class _PerfilFormCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final visibleInteressesCount = (interessesPageSize * interessesPagesShown).clamp(0, disciplinas.length);
    final visibleInteresses = disciplinas.take(visibleInteressesCount).toList(growable: false);
    final hasMoreInteresses = disciplinas.length > visibleInteressesCount;

    final displayName = '${primeiroNomeController.text.trim()} ${ultimoNomeController.text.trim()}'.trim();
    final headerName = displayName.isEmpty ? '—' : displayName;
    final headerEmail = emailController.text.trim().isEmpty ? '—' : emailController.text.trim();
    final headerInitials = _getInitials(displayName.isEmpty ? usernameController.text : displayName);
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _PerfilHeader(initials: headerInitials, name: headerName, email: headerEmail),
          const SizedBox(height: PerfilConstants.cardGap),
          PerfilLabeledField(label: 'Username', controller: usernameController, readOnly: true),
          const SizedBox(height: PerfilConstants.fieldGap),
          Row(
            children: [
              Expanded(
                child: PerfilLabeledField(label: 'Primeiro nome', controller: primeiroNomeController),
              ),
              const SizedBox(width: 16.685),
              Expanded(
                child: PerfilLabeledField(label: 'Último nome', controller: ultimoNomeController),
              ),
            ],
          ),
          const SizedBox(height: PerfilConstants.fieldGap),
          ...
          if (!disciplinasLoading && disciplinasError == null && hasMoreInteresses) ...[
            ...
            Align(
              ...
              child: OutlinedButton(
                ...
                style: OutlinedButton.styleFrom(
                  side: const BorderSide(color: PerfilConstants.orange, width: 2),
          ...
          InkWell(
            ...
            child: Container(
              ...
              child: const Text('Salvar Perfil', style: PerfilConstants.buttonTextStyle),
            ),
          ),
                  ...
                  Positioned(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: Colors.white,
                        shape: BoxShape.circle,
                        border: Border.all(color: PerfilConstants.cardBorderColor, width: 2.781),
                        boxShadow: PerfilConstants.floatingShadow,
                      ),
                      child: const Center(
                        child: Icon(Icons.edit, size: 27.809, color: Color(0xFF364153)),
                      ),
                    ),
                  ),
                ],
...
String _getInitials(String input) {
  ...
  final parts = cleaned.split(RegExp(r'\s+')).where((p) => p.trim().isNotEmpty).toList(growable: false);
  if (parts.isEmpty) return '—';
  if (parts.length == 1) {
    final w = parts.first;
    return w.length >= 2 ? w.substring(0, 2).toUpperCase() : w.substring(0, 1).toUpperCase();
  }
  final a = parts.first;
  final b = parts.last;
