...
DROP FUNCTION IF EXISTS public.usp_areas_select_all01(varchar);
...
DROP FUNCTION IF EXISTS public.usp_areas_insert01(character varying, text, varchar);
DROP FUNCTION IF EXISTS public.usp_areas_update01(uuid, character varying, text);
DROP FUNCTION IF EXISTS public.usp_areas_update01(uuid, character varying, text, varchar);
...
DROP FUNCTION IF EXISTS public.usp_areas_ensure01(character varying, text, varchar);

CREATE OR REPLACE FUNCTION public.usp_areas_select_all01(
  p_target_role varchar(50) DEFAULT NULL
)
...
CREATE OR REPLACE FUNCTION public.usp_areas_insert01(
  p_nome varchar(150),
  p_descricao text,
  p_target_role varchar(50) DEFAULT 'ensino'
)
RETURNS uuid
LANGUAGE plpgsql
AS $$
DECLARE
  v_id uuid;
BEGIN
  INSERT INTO public.areas (nome, descricao, target_role, created_at, updated_at)
  VALUES (p_nome, p_descricao, p_target_role, now(), now())
